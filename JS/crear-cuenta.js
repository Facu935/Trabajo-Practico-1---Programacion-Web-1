...
function crearCuenta(formulario) {
  formulario.addEventListener("submit", (event) => {
    ...
    const aceptacionTerminos =
      document.getElementsByName("acepto-terminos")[0].checked;
    ...
    const especialesPass = password.replace(
      new RegExp(
        `[^${especiales.replace(/[\!\$\%\&\/\(\)\=\?\¿\¡]/g, "\\$&")}]`,
        "g"
      ),
      ""
    );
    ...
    if (password.length < 8)
      errores.push("La contraseña al menos debe tener 8 caracteres");
    if (letras.length < 2)
      errores.push("La contraseña debe contener al menos 2 letras");
    if (numeros.length < 2)
      errores.push("La contraseña debe contener al menos 2 números");
    if (especialesPass.length < 2)
      errores.push(
        `La contraseña le falta alguno de los siguientes caracteres especiales: ${especiales}`
      );
