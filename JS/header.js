import {
    validarUsuarioConectadoParaNav,
    obtenerUsuarioLogueado,
    limpiarUsuarioLogueado,
    guardarModificacionLocalStorage,
    guardarModificacionLocalStorageUsuarioLogueado,
    limpiarLocalStorage,
    ...
    contador.addEventListener("click", () => {
        ...
        for (let i = 0; i < cursosEnCarritoDelUsuario.length; i++) {
            ...
            const templateCursos = `<div class="recuadro-curso-sidebar" id="recuadro-sidebar${i + 1
                }"><div class="titulo-curso-sidebar">
                                                        <img src="${curso.img
                }" alt="Logo ${curso.nombre
                }" id="logo-curso-sidebar">
                                                        <h3>${curso.nombre}</h3>
                                                        </div>
                                                        <div class = "recuadro-curso-sidebar segundo-renglon">
                                                            <ul>
                                                                <li id="valoracion-curso-carrito">Valoracion:<img src="../IMG/Cursos/estrella.png" alt="" id> ${curso.valoracion
                }</li>
                                                                <li id="precio-curso-carrito">Precio: $ ${curso.precio
                }</li>
                                                                <li>Cantidad: ${curso.cantidad
                }</li>
                                                                <li>Modalidad: Virtual</li>
                                                            </ul>
                                                            <button class="boton-eliminar-curso-carrito" id="boton-eliminar${i + 1
...
function eliminarCursoDelSideBar(cursosEnCarritoDelUsuario, contenedor) {
    for (let i = 0; i < cursosEnCarritoDelUsuario.length; i++) {
        const CURSO_A_BORRAR = document.querySelector(
            "#recuadro-sidebar" + (i + 1)
        );
        const BOTON_ELIMINAR_CORRECTO = document.querySelector(
            "#boton-eliminar" + (i + 1)
        );
...
function mostrarBienvenidoUsuario(contenedor) {
    ...
    BIENVENIDO_USUARIO.textContent =
        "Bienvenido, " + obtenerUsuarioLogueado().nombre;
